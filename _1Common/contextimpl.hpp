...
static inline DWORD symbol(LPCSTR ptr) {
    return static_cast<DWORD>(static_cast<BYTE>(*ptr));
}
...
struct simplefnd_pol {
    ...
    template<>static int find<first_def>(LPCSTR buf, first_def* p) {
        return -1;
    }
};
...
struct hs_find_nearest_bigger {
    template<int I>	struct hashsize {
        enum {val = hashsize<I + 1>::val};
    };
...
template<BOOL I> struct hs_selector {
    typedef hs_find_nearest_bigger type;
};
template<> struct hs_selector<FALSE> {
    typedef hs_simple_calc type;
};

template<typename T, int I> struct hs_get {
    enum{val = T::hashsize<I>::val};
};
...
struct hashfnd_pol {
    ...
    hashfnd_pol() {
        memset(ppHashTable, 0, sizeof(ppHashTable));
    }
    ~hashfnd_pol() {
        int cnt=Count*2;
    ...
    static hashfnd_pol<T>*& getme() {
        static hashfnd_pol<T>* ptr;
        return ptr;
    }
...
struct fndselector {
    ...
    template<>struct tdef<0> {
        typedef str_find::nofnd_pol<T>		type;
    };
    template<>struct tdef<1> {
        typedef str_find::onefnd_pol<T>	type;
    };
    template<>struct tdef<2> {
        typedef str_find::simplefnd_pol<T>	type;
    };
    template<>struct tdef<3> {
        typedef str_find::hashfnd_pol<T>	type;
    };
...
struct getname {
    ...
    struct finder {
        ...
        static finder<Count>* getme() {
            static finder<Count>* one=new finder<Count>;
            return one;
        }
        finder() {
            fill((T::prev*)NULL);
...
struct meth_count {
    ...
    template<> struct counter<0> {
        template<typename M>struct check {
            enum {val = 0};
        };
    };
...
template<typename T>
struct some_np {
    template<int I>	struct _some_np {
        template<typename M>struct check {
            enum {val=I};
        };
    };
    template<> struct _some_np<-1> {
        template<typename M>struct check {
            enum {val=-1};
        };
    };
...
struct meth_pol {
    ...
    struct policy {
        ...
        template<> static int HasRetVal<first_def>(DWORD, first_def*) {
            return FALSE;
        }
    };
    template<>
    ...
    template<int I>struct nparam {
        static int GetNParams(DWORD) {
            return I;
        }
    };
    ...
    struct nparam<-1> {
        ...
        template<>static int _GetNParams<first_def>(DWORD, first_def*) {
            return -1;
        }
    };
    ...
    template<>static int GetDefVal<first_def>(const T*, DWORD, DWORD, CValue*, first_def*) {
        return FALSE;
    }
    ...
    template<>static int CallAsProc<first_def>(T*, DWORD, CValue**, first_def*) {
        return FALSE;
    }
    ...
    template<>static int CallAsFunc<first_def>(T*, DWORD, CValue&, CValue**, first_def*) {
        return FALSE;
    }
};
...
struct prop_pol {
    ...
    struct policy {
        ...
        template<>static int IsRead<first_def>(DWORD, first_def*) {
            return FALSE;
        }
        ...
        template<>static int IsWrite<first_def>(DWORD, first_def*) {
            return FALSE;
        }
    };
    template<>
    ...
    template<>static int ReadProp<first_def>(const T*, DWORD, CValue&, first_def*) {
        return FALSE;
    }
    ...
    template<>static int WriteProp<first_def>(T*, DWORD, CValue const&, first_def*) {
        return FALSE;
    }
};
...
struct implement_rtc {
    static CObject* PASCAL createobject_rtc() {
        return new T;
    }
    static CRuntimeClass* PASCAL GetBaseRTC() {
...
struct reg_CreateObject {
    ...
    static void Done() {
        CBLContext::UnRegisterContextClass(T::GetRTC());
    }
};
...
class CContextImpl: public CContextBase {
  public:
    ...
    CContextImpl() {
        static init_all_map iam;
    }
    virtual ~CContextImpl() {}


    template<typename P> struct end_def {
        typedef P prev;
        enum {val=prev::val+1};
    };
    ...
    static void Init() {
        init_done_policy::Init();
    }
    static void Done() {
        init_done_policy::Done();
    }
};
...
struct variable_arguments {
    ...
    template<>static int _GetNParams<first_def>(DWORD, const DWORD&, first_def*) {
        return -1;
    }
};
